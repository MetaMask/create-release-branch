...
async function planRelease(
  ...
  const workspaceReleasePlans: PackageReleasePlan[] = Object.keys(
    ...
    return {
      manifestFile,
      packageMetadata: {
        dirPath: manifestFile.parentDirectory,
        manifest: {
          ...manifestFile.data,
          version: manifestFile.data.version.toString(),
        },
      },
      updateSpecification: {
        newVersion,
...
async function applyUpdatesToMonorepo(
  ...
  await Promise.all(
    releasePlan.packages.map(
      async ({ packageMetadata, updateSpecification }) => {
        debug(`Updating package ${packageMetadata.manifest.name}...`);
        await updatePackage(
          packageMetadata,
          updateSpecification,
