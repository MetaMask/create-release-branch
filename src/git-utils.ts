...
export async function runGitCommandWithin(
  ...
  await runCommand('git', args, { cwd: repositoryDirectoryPath });
}
...
export async function getStdoutFromGitCommandWithin(
  ...
  return await getStdoutFromCommand('git', args, {
 ...
 * @throws An execa error object if the command fails in some way.
 */
export async function getLinesFromGitCommandWithin(
  repositoryDirectoryPath: string,
  args: readonly string[],
): Promise<string[]> {
  return await getLinesFromCommand('git', args, {
    cwd: repositoryDirectoryPath,
  });
}
...
async function getFilesChangedSince(
  ...
  return await getLinesFromGitCommandWithin(repositoryDirectoryPath, [
    'diff',
    tagName,
    'HEAD',
    '--name-only',
...
export async function getTagNames(
  ...
  const sortedTagNames = await getLinesFromGitCommandWithin(
    repositoryDirectoryPath,
    ['rev-list', '--tags', '--date-order'],
  );

  if (unsortedMergedTagNames.length === 0 && sortedTagNames.length === 0) {
    if (await hasCompleteGitHistory(repositoryDirectoryPath)) {
      return [];
    }
  ...
  return placeInSpecificOrder(unsortedMergedTagNames, sortedTagNames);
}
